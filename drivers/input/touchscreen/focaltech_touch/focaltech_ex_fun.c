...
static ssize_t fts_bl_flag_store(
    struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{	
	sscanf(buf,"%d",&fts_bl_flag);
    return count;
}

static ssize_t ffts_bl_flag_show(
    struct device *dev,
...
static DEVICE_ATTR(fts_bl_flag, S_IRUGO | S_IWUSR, ffts_bl_flag_show, fts_bl_flag_store);
